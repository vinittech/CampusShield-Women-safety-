 ...
 * Includes Student & Admin Authentication System, Trust Score Engine, and Offline Sync Endpoint
 */
...
struct User {
    ...
    string password;
    string name;
    string role; // "student", "admin", "security"
    string collegeId;
...
// Initialize Default Campus Data & Pre-registered Accounts
void initCampusData() {
    // Pre-registered Student & Admin Accounts
    g_users.push_back({"student@university.edu", "student123", "Priya Sharma", "student", "STU-2026-8891", 95});
    g_users.push_back({"admin@university.edu", "admin123", "Dr. A. K. Verma (Chief Warden)", "admin", "ADM-2026-0001", 100});
    g_users.push_back({"security@university.edu", "security123", "Inspector R. Singh (Head Security)", "security", "SEC-2026-0042", 100});
...
// Helper to extract JSON parameter value
...
void runServer(int port) {
    ...
    cout << "==========================================================" << endl;
    cout << "  CampusShield C++ Auth & AI Server Running on Port " << port << endl;
    cout << "  Student Login: student@university.edu / student123" << endl;
    cout << "  Admin Login:   admin@university.edu   / admin123" << endl;
    cout << "==========================================================" << endl;
    ...
    while (true) {
        ...
        if (bytesRead > 0) {
            ...
            else if (request.find("POST /api/login") != string::npos || request.find("GET /api/login") != string::npos) {
                string email = extractJsonField(request, "email");
                string password = extractJsonField(request, "password");

                if (email.empty()) {
                    // Fallback to query param extraction
                    size_t posE = request.find("email=");
                    if (posE != string::npos) {
                        size_t endE = request.find_first_of(" &\r\n", posE);
                        email = request.substr(posE + 6, endE - (posE + 6));
                    }
                    size_t posP = request.find("password=");
                    if (posP != string::npos) {
                        size_t endP = request.find_first_of(" &\r\n", posP);
                        password = request.substr(posP + 9, endP - (posP + 9));
                    }
                }

                User* user = authenticateUser(email, password);
                if (user) {
                    string body = "{\"status\":\"success\",\"message\":\"Authentication successful\",\"user\":{\"email\":\"" + user->email + "\",\"name\":\"" + user->name + "\",\"role\":\"" + user->role + "\",\"collegeId\":\"" + user->collegeId + "\",\"trustScore\":" + to_string(user->trustScore) + "}}";
                    response = buildHttpResponse(body);
                } else {
                    string body = "{\"status\":\"error\",\"message\":\"Invalid college email or password.\"}";
                    response = buildHttpResponse(body, "application/json", 401);
                }
            }
            // User Register API Endpoint
                ...
                string role = extractJsonField(request, "role");
                if (role.empty()) role = "student";

                if (!email.empty() && !password.empty()) {
                    g_users.push_back({email, password, name, role, "STU-2026-" + to_string(rand() % 9000 + 1000), 90});
                    string body = "{\"status\":\"success\",\"message\":\"Account created successfully.\"}";
                    response = buildHttpResponse(body);
                } else {
                    response = buildHttpResponse("{\"status\":\"error\",\"message\":\"Missing email or password.\"}", "application/json", 400);
                }
            }
            // API Health Endpoint
            else if (request.find("GET /api/health") != string::npos) {
                string body = "{\"status\":\"online\",\"engine\":\"CampusShield C++ Auth & AI Server\",\"version\":\"v3.0 (Student & Admin Auth Enabled)\",\"totalUsers\":" + to_string(g_users.size()) + ",\"totalIncidents\":" + to_string(g_incidents.size()) + "}";
                response = buildHttpResponse(body);
            }
